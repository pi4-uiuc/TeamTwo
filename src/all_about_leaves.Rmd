...
leaf_variables <- select(readRDS('data/variables.rds'), variable_id, description, name, units) %>%    # Only care about the important stuff
...
leaf_traits_bare <- readRDS('data/traits.rds') %>%
...
results <- predict_species(leaf_biomass_plant = 4.3, leaf_respiration_rate_m2 =   1.18)
...
```{r}
resprateN <- subset(leaf_traits, name == 'leafN' | name == 'leaf_respiration_rate_m2')
