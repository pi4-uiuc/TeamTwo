...
leaf_traits <- betydb_query(table = "traits", limit = 3000) %>%
...
Based on what traits and species we chose to query, it looks like Salix polaris is the most represented. Let's choose only the Salix polaris data, and then group by variable measured:

```{r}

salpol_data <- leaf_traits %>%
  filter(scientificname == "Salix polaris") %>%
  group_by(name) %>%
  summarize(n = n())
