...
predict_species <- function(c2n_leaf = NA, leafN = NA, leaf_biomass_area = NA, leafP = NA, leaf_respiration_rate_m2 = NA, leaf_longevity = NA, leafC = NA, leaf_biomass_plant = NA, leaf_turnover_rate = NA) {
  prediction_key = c(c2n_leaf, leafN, leaf_biomass_area, leafP, leaf_respiration_rate_m2, leaf_longevity, leafC, leaf_biomass_plant, leaf_turnover_rate)
  n = nrow(final_table)
  distance_column = data.frame(matrix(ncol = 1, nrow = n))
  colnames(distance_column) = "Difference"
  for(i in 1:nrow(final_table)){
    distance_column[i,1] = species_distance(final_table[i,2:10], prediction_key)
  }
