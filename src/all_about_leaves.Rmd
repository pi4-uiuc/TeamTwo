...
leaf_variables <- loadRDS("data/variables.rds") %>%
...
leaf_traits_bare <- loadRDS('data/traits.rds') %>%
...
Now we will split our data into training data (90%) and testing data (10%) after randomizing it by row.
...
predict_species <- function(c2n_leaf = NA, leaf_biomass_area = NA, leaf_biomass_plant = NA, leaf_longevity = NA, leaf_respiration_rate_m2 = NA, leaf_turnover_rate = NA, leafC = NA, leafN = NA, leafP = NA) {
...
test_predictor <- function(species, traitsN) {
  c2n_leaf <- NA
  leaf_biomass_area = NA
  leaf_biomass_plant = NA
  leaf_longevity = NA
  leaf_respiration_rate_m2 = NA
  leaf_turnover_rate = NA
  leafC = NA
  leafN = NA
  leafP = NA
  ...
  if(nrow(take_from) < traitsN) {
    ...
    print(take_from[1:3,])
    variables = variableHelper(take_from, traitsN)
    c2n_leaf = variables[1]
    leaf_biomass_area = variables[2]
    leaf_biomass_plant = variables[3]
    leaf_longevity = variables[4]
    leaf_respiration_rate_m2 = variables[5]
    leaf_turnover_rate = variables[6]
    leafC = variables[7]
    leafN = variables[8]
    leafP = variables[9]
    return(predict_species(c2n_leaf, leaf_biomass_area, leaf_biomass_plant, leaf_longevity, leaf_respiration_rate_m2, leaf_turnover_rate, leafC, leafN, leafP))
  }
}

variableHelper <- function(take_from, traitsN, c2n_leaf = NA, leaf_biomass_area = NA, leaf_biomass_plant = NA, leaf_longevity = NA, leaf_respiration_rate_m2 = NA, leaf_turnover_rate = NA, leafC = NA, leafN = NA, leafP = NA) {
  if(traitsN == 0) {
    return(c(c2n_leaf, leaf_biomass_area, leaf_biomass_plant, leaf_longevity, leaf_respiration_rate_m2, leaf_turnover_rate, leafC, leafN, leafP))
  }
  else {
    firstVariable = take_from[1,"name"]
    value = take_from[1,"mean"]
    if(firstVariable == "c2n_leaf") {
      c2n_leaf = value
    }
    else if(firstVariable == "leaf_biomass_area") {
      leaf_biomass_area = value
    }
    else if(firstVariable == "leaf_biomass_plant") {
      leaf_biomass_plant = value
    }
    else if(firstVariable == "leaf_longevity") {
      leaf_longevity = value
    }
    else if(firstVariable == "leaf_respiration_rate_m2") {
      leaf_respiration_rate_m2 = value
    }
    else if(firstVariable == "leaf_turnover_rate") {
      leaf_turnover_rate = value
    }
    else if(firstVariable == "leafC") {
      leafC = value
    }
    else if(firstVariable == "leafN") {
      leafN = value
    }
    else if(firstVariable == "leafP") {
      leafP = value
    }
    return(variableHelper(take_from[-1,], traitsN - 1, c2n_leaf, leaf_biomass_area, leaf_biomass_plant, leaf_longevity, leaf_respiration_rate_m2, leaf_turnover_rate, leafC, leafN, leafP))
  }
}

test_predictor("Acer saccharum", 3)
